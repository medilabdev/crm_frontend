...
const EditContact = () => {
  ...
  const removeTelephone = (telephone) => {
    if(telephone){
      Swal.fire({
        ...
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/contacts/delete/item/telp/${telephone}`, formData,{
          headers:{
            Authorization:`Bearer ${token}`
          }
        }
  ...
  const selectCompany = () => {
    ...
    const filteredCompanies = res.filter(
      (company) => !oldComp.some((oldCompany) => oldCompany?.company?.uid === company.value)
    );
  ...
  const selectDeals = () => {
    ...
    const filteredDeals = res.filter((deals) => !oldComp.some((old) => old?.deals?.uid === deals.value))
  ...
  const handleDeleteDeals = (uid) => {
    ...
    Swal.fire({
      ...
      if(res.isConfirmed){
        ...
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/contacts/delete/item/deals`,formData, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        }).then(() => {
  ...
  const handleDeleteComp = (uid) => {
    Swal.fire({
      ...
      if(res.isConfirmed){
        const formData = new FormData();
        formData.append("associate_uid", uid)
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/contacts/delete/item/company`,formData, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        }).then(() => {
          ...
          <div className="row">
              ...
              <Card className="shadow">
                ...
                <Card.Body>
                  <FloatingLabel
                    ...
                    className="mb-3"
                  >
                    <Form.Control type="text" name="name" value={editContact.name} placeholder="name@example.com" onChange={handleInput} />
                  </FloatingLabel>
                  <FloatingLabel
                    controlId="floatingInput"
                    label="Job Title"
                    className="mb-3"
                  >
                    <Form.Control type="text" name="position" value={editContact.position} placeholder="name@example.com" onChange={handleInput}  />
                  </FloatingLabel>
                  <Form.Label>
                      Owner
                    ...
                    <Select className="mb-3" required options={selectOwner()} value={selectOwner().find((e) => e.value === editContact.owner_user_uid)} onChange={(e) => setEditContact({
                      ...editContact,
                    ...
                    <Form.Control type="email" name="email" value={editContact.email} onChange={handleInput} placeholder="@gmail.com" />
                    </Form.Group>
                  ...
                  <FloatingLabel
                    ...
                    label="Address"
                  >
                    <Form.Control type="text" value={editContact.address} onChange={handleInput} placeholder="@gmail" />
                  </FloatingLabel>
                  <FloatingLabel
                    className="mb-3"
                    controlId="floatingInput"
                    label="City"
                  >
                    <Form.Control type="text" name="city" value={editContact.city} onChange={handleInput} placeholder="@gmail" />
                  </FloatingLabel>
                  <FloatingLabel
                    className="mb-3"
                    controlId="floatingInput"
                    label="Zip Code"
                  >
                    <Form.Control type="number" name="postal_code" value={editContact.postal_code} onChange={handleInput} placeholder="@gmail" />
                  </FloatingLabel>
                  <FloatingLabel
                    className="mb-3"
                    controlId="floatingInput"
                  ...
                  <Form.Group className="mb-3">
                    <Form.Label>
                      Source <span style={{ color: "red" }} className="fs-6">
                        *
                      </span>
                    </Form.Label>
                    <Select options={selectSource()} required value={selectSource().find((e) => e.value === editContact.source_uid)} onChange={(e) => setEditContact({
                      ...editContact,
                      source_uid:e.value
                    })} />
                  </Form.Group>
                  <Form.Select
                    ...
                    value={editContact.gender} onChange={(e) => setEditContact({ ...e, gender:e.target.value})}
                  >
                    <option>Sex</option>
                  ...
                  <Form.Control
                      type="text"
                      name="remarks"
                      value={editContact.remarks} onChange={handleInput}
                    />
                  </Form.Group>
                </Card.Body>
              </Card>
                        ...
                        <div className="row d-flex">
                          <>
                            ...
                            <Col md={2} sm={2} style={{ marginTop: "-8px" }}>
                              <a onClick={() => handleDeleteDeals(data.uid)} className="ms-1" style={{
                                  cursor: "pointer",
                                }}><i className="bi bi-x fs-5 text-danger"></i></a>
                            </Col>
                            </>
                        </div>
                      </Card>
              ...
              <Card className="shadow">
                ...
                <Card.Body>
                  <div>
                      ...
                      data.company ? (
                        ...
                        <div className="row d-flex">
                          <>
                            <Col md={10} sm={10} key={index}>
                              <div className="d-flex">
                                <img src={IconCompany} alt={IconCompany} className="ms-1 me-3 mt-3" style={{ width:"30px", height: "30px" }}/>
                              
                              <Col md={12} sm={12}>
                              <p className="ms-1 mt-2 me-3" style={{
                                      fontSize: "10px",
                                      whiteSpace: "normal",
                                    }}> Name : <strong className="ms-2">{data?.company?.name ?? null}</strong> </p>
                                    <p className="ms-1" style={{
                                      fontSize: "10px",
                                      marginTop: "-8px",
                                    }}>Badan : <strong className="ms-2">{data?.company?.company_type?.name}</strong></p>
                            </Col>
                            </div>
                            </Col>
                            <Col md={2} sm={2} style={{ marginTop: "-8px" }}>
                              <a onClick={() => handleDeleteComp(data.uid)} className="ms-1" style={{
                                  cursor: "pointer",
                                }}><i className="bi bi-x fs-5 text-danger"></i></a>
                            </Col>
                            </>
                    ...
                    <Select options={selectCompany()} closeMenuOnSelect={false} isMulti onChange={(e)=>handleCompany(e)} placeholder="Select Company"/>
            ...
            <div className="col-md-8">
              <Card className="shadow mb-2">
                ...
                <Card.Body>
                  <ReactQuill className="p-2" theme="snow" name="notes" onChange={(value) => handleInput({target: {name: "notes", value}})} />
                </Card.Body>
              </Card>
              <Card className="shadow">
                  <Card.Header> <h6 className="fw-bold mt-2">History</h6></Card.Header>
                  <Card.Body>
                    <DataTable data={history} columns={historyColumns} pagination paginationComponentOptions={paginationComponentOptions} defaultSortFieldId={1} customStyles={customStyles} />
                  </Card.Body>
              </Card>
            </div>
            </form>
