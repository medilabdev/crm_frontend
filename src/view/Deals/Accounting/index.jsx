...
// remove payment
  ...
  const getExpCat = () => {
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/expense-categories`, {
      headers:{
  ...
  const getDataPayment = (token, uid) => {
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/payments/${uid}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
  ...
  const getDataExpanse = (token, uid) =>{
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/expenses/${uid}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
  ...
  const handleSubmit = (e) => {
    ...
    inputPaymentAmount.forEach((amount, index) => {
      formData.append(`payment[${index}][payment_amount]`, amount.payment)
    })
    ...
    inputPaymentReff.forEach((payReff, index) => {
      formData.append(`payment[${index}][references_number]`, payReff.reff_number || "")
    })
  ...
  const handleDeletePayment = (uid) => {
    Swal.fire({
      ...
      if(res.isConfirmed){
        axios.delete(`${process.env.REACT_APP_BACKEND_URL}/payments/delete/${uid}`,{
          headers:{
            Authorization:`Bearer ${token}`
          }
        })
  ...
  const colPayment = [
    ...
    {
      name: "Payment Amount", 
      selector: (row) => `Rp. ${new Intl.NumberFormat().format(row.payment_amount)}`,
      sortable: true
    },
    {
      name: "Payment Method", 
      selector: (row) => `${row.payment_method.charAt(0).toUpperCase()}${row.payment_method.slice(1)}`
    ...
    },
    {
      ...
      selector:(row) => (
        <div>
          <button title="delete" className="icon-button" onClick={() => handleDeletePayment(row.uid)}>
      ...
      <Main>
        <div className="container">
          ...
          <div className="row">
                  ...
                  {payment.map((pay, index) => (
                    <>
                     ...
                     <Form.Group className="mb-3">
                       ...
                       <Form.Control type="date" onChange={(e) => handleInputPaymentDate(e, index)} required />
                     </Form.Group>
                     <Form.Group className="mb-3">
                       <Form.Label style={{ fontWeight: 600 }}>
                         Payment Amount
                       ...
                       <InputGroup>
                         ...
                         <Form.Control type="number" name="payment"  required onChange={(e) => handleInputPaymentAmount(e, index)} />
                       </InputGroup>
                     </Form.Group>
                     <Form.Group className="mb-3">
                       <Form.Label style={{ fontWeight: 600 }}>
                       ...
                       <select className="form-select" onChange={(e) => handleInputPaymentMethod(e, index)} required>
                     ...
                     <Form.Group className="mb-3">
                       ...
                       <Form.Control type="text" name="reff_number" onChange={(e) =>handleInputPaymentReff(e, index)} />
                     </Form.Group>
                     {payment.length > 1 && (
              ...
              <Card className="shadow">
                ...
                <Card.Body>
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control type="date" onChange={(e) => handleInputExpanseDate(e, index)} />
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label style={{ fontWeight: 600 }}>
                      Expense Amount
                    ...
                    <InputGroup>
                      <InputGroup.Text>Rp.</InputGroup.Text>
                      <Form.Control type="number" name="expAmo" onChange={(e) => handleInputExpanseAmount(e, index)} />
                    </InputGroup>
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label style={{ fontWeight: 600 }}>
                    ...
                    <select className="form-select" onChange={(e) => handleInputExpanseCat(e, index)}>
                      <option disabled selected>
                        Select Choose
                      </option>
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control as="textarea" name="reff_exp" onChange={(e) =>handleInputExpanseReff(e, index)}  />
                  </Form.Group>
                  {expanse.length > 1 && (
            ...
            <div className="col-md-12">
              <Card className="shadow">
                <DataTable className="mt-4" data={dataPayment} columns={colPayment} title="Payment" customStyles={customStyles} />
              </Card>
            </div>
            : null}
            ...
            <div className="col-md-12">
            <Card className="shadow">
              <DataTable className="mt-4" data={dataExpanse}  title="Expanse" customStyles={customStyles} />
            </Card>
          </div>
            : null} 
          </div>
